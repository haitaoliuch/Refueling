...
switch options
    ...
    for i =1:pro.T-1
        pro.D(i,i+1)=com(i+1);
        pd = fitdist(DataCs(i,:)','Gamma');
        pro.shape(i) = pd.a ; % Mean of Price
        pro.scale(i)= pd.b; % Variance of Price
    end
    pro.IniInventory = 0:10:90;
    case 'S5' % S instances to study the sensitivity to fuel prices
    pro.T =16;  % number of stage
    pro.C =300; % bunker capacity
    pro.L  =0;  % safe bunker volume for the container ship
    pro.fc =ones(pro.T,1)*20; % fixed cost of bunker repleshiment at each stage
    pro.D  = d(1:pro.T,1:pro.T); % bunker consumption matrix
    pro.shape = shape(1:pro.T) ; pro.shape(1) = 16; 
    pro.scale = scale(1:pro.T);  pro.scale(1) = 2; 
    pro.IniInventory = [0:10:90];
    case 'S6' % S instances to study the sensitivity to fuel prices
    pro.T =16;  % number of stage
    pro.C =300; % bunker capacity
    pro.L  =0;  % safe bunker volume for the container ship
    pro.fc =ones(pro.T,1)*20; % fixed cost of bunker repleshiment at each stage
    pro.D  = d(1:pro.T,1:pro.T); % bunker consumption matrix
    pro.shape = shape(1:pro.T) ; pro.shape(1) = 16; 
    pro.scale = scale(1:pro.T);  pro.scale(1) = 4; 
    pro.IniInventory = [0:10:90];
    case 'S7' % S instances to study the sensitivity to fuel prices
    pro.T =16;  % number of stage
    pro.C =300; % bunker capacity
    pro.L  =0;  % safe bunker volume for the container ship
    pro.fc =ones(pro.T,1)*20; % fixed cost of bunker repleshiment at each stage
    pro.D  = d(1:pro.T,1:pro.T); % bunker consumption matrix
    pro.shape = shape(1:pro.T) ; pro.shape(1) = 16; 
    pro.scale = scale(1:pro.T);  pro.scale(1) = 6; 
    pro.IniInventory = [0:10:90];
    case 'S8' % S instances to study the sensitivity to fuel prices
    pro.T =16;  % number of stage
    pro.C =300; % bunker capacity
    pro.L  =0;  % safe bunker volume for the container ship
    pro.fc =ones(pro.T,1)*20; % fixed cost of bunker repleshiment at each stage
    pro.D  = d(1:pro.T,1:pro.T); % bunker consumption matrix
    pro.shape = shape(1:pro.T) ; pro.shape(1) = 16; 
    pro.scale = scale(1:pro.T);  pro.scale(1) = 8; 
    pro.IniInventory = [0:10:90];

    case 'CS'
    pro.T =17;  % number of stage
    pro.C =3000; % bunker capacity
    pro.L  =0;  % safe bunker volume for the container ship
    pro.fc =ones(pro.T,1)*5000; % fixed cost of bunker repleshiment at each stage
    com =[0 140.49	60.85	180.49	120.4	21.76	485.95	1855.94	295.66	20.66	76.69	116.61	391.83	229.1	526.91	480.3	388.72];
    pro.D  = zeros(pro.T,pro.T); % bunker consumption matrix
