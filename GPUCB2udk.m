function [reward,time,GP]=GPUCB2udk(N,P,R)
...
kfcn = @(x1,x2,theta) (exp(theta(4))^2)*exp(-1/2*((pdist2(x1(:,1),x2(:,1))).^2/(exp(theta(1))^2)+(pdist2(x1(:,2),x2(:,2))).^2/(exp(theta(2))^2)+...
    (pdist2(x1(:,3:end),x2(:,3:end)).^2)/(exp(theta(3))^2)));
theta0=[1,1,1,1];
...
for n=1:N 
    ...
    parfor t=1:pro.T-1 
        % if mod(n,10)==0
        GP{t} = fitrgp([Sample{t},Price(:,t+1:end)],cr(:,t),'FitMethod','exact','PredictMethod','exact','KernelFunction',kfcn,'KernelParameters',theta0); 
    end 
     time(n)=toc;
    n
end
