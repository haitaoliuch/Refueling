function [reward,est_price,GP]=GPAOAPfp(N,P,R)
...
%%Note: to test seasonality, the initial fuel price at training and
%%testing, and Problem is put outside
%%are changed
% suppose we can observe all fuel prices at all ports
pro = P;
k_max =pro.T+4;  Price=[];
for n=1:N
   s_t = [rand*pro.C,gamrnd(pro.shape(1),pro.scale(1))]; 
    for t=1:pro.T-1              
        ...
        if n<=  k_max+1
           % Price(n,t) = s_t(2);
            [~,price] =TransitionCor(s_t,0,t,pro); % drwa fuel price observations
            Price=[Price;price(1:pro.T-1)];
            [a] = A(randi(length(A)));
        else 
            [~,price] =TransitionCor(s_t,0,t,pro); % drwa fuel price observations
            Price=[Price;price(1:pro.T-1)];
            ...
            parfor k = 1 : k_max
                GMM{k} = fitgmdist(Price,k,'Options',options,'RegularizationValue',0.1);
                AIC(k)= GMM{k}.AIC;
            end
            [~,k_best] = min(AIC);
            [est_price] = GMR(GMM{k_best}, Price(:,1:t)', [1:t], [t+1:pro.T-1]);       
...
for i = 1:length(pro.IniInventory)
    parfor r = 1:R
        s_t = [pro.IniInventory(i),100];
        cost=[]; P_n= zeros(1,pro.T);
        for t=1:pro.T-1 
            P_n(t) = s_t(2);
            ...
            s_t = TransitionSeasonal(s_t,a,t,pro); 
        end 
        Obj(r,:)= cost;
    end
    mObj(i,:) = mean(Obj,1);
